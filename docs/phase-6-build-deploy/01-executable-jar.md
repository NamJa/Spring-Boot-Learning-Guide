...
모든 배포는 **하나의 실행 가능한 산출물**에서 출발합니다. Spring Boot의 Gradle 플러그인은 애플리케이션 코드와 모든 의존성을 단일 jar에 담은 **실행 가능 jar**(흔히 *fat jar* 또는 *uber jar*)를 만들어 줍니다. 별도의 톰캣 설치 없이 `java -jar`만으로 내장 톰캣(Tomcat 11.0.22)을 띄우고 8080 포트에서 서비스가 시작됩니다.
