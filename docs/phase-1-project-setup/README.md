...
> 💡 이 가이드는 2026-07-25 기준 최신 GA 버전인 **Spring Boot 4.1.0** (2026-06-10 출시), **Spring Framework 7.0.8**, **Kotlin 2.3.21** (Spring Boot BOM 관리)을 기준으로 합니다. JDK는 LTS인 **21**로 표준화합니다.
>
> ⚠️ Spring Boot 4는 스타터 이름과 JSON 엔진이 3.x와 다릅니다(`spring-boot-starter-web` → **`spring-boot-starter-webmvc`**, Jackson 2 → **Jackson 3**). 4번 페이지에서 Initializr가 실제로 생성하는 `build.gradle.kts`를 그대로 놓고 하나씩 설명합니다.
