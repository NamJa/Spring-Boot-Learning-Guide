...
> [!WARNING]
> ThreadLocal은 **별도 스레드로 작업을 넘기면 기본적으로 전파되지 않습니다.** 트랜잭션·보안 컨텍스트가 새 스레드에서 사라지는 사고의 원인이 되니, 비동기 경계에서는 컨텍스트 전파(`TaskDecorator` 등)를 챙겨야 합니다.
>
> 다만 **Spring Boot 4.1부터는 `@Async` 메서드에 컨텍스트가 자동 전파**됩니다(Micrometer Context Propagation 연동). `@Async`를 쓴다면 4.1 이상에서는 이 함정이 상당 부분 해소되지만, 직접 만든 스레드 풀이나 코루틴 디스패처로 넘길 때는 여전히 수동으로 챙겨야 합니다.
