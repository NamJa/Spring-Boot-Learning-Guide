...
- **모든 엔드포인트가 인증을 요구**합니다. `GET /books`조차 401이 납니다.
...
- `GET /books`, `GET /books/{id}` → **누구나** 허용
- `POST`, `PUT`, `DELETE /books/**` → **인증된 사용자만**
- 그 외는 인증 요구
    ...
    fun securityFilterChain(http: HttpSecurity): SecurityFilterChain {
        http {
            ...
            authorizeHttpRequests {
                ...
                // 변경은 인증 필요
                authorize(HttpMethod.POST, "/books/**", authenticated)
                authorize(HttpMethod.PUT, "/books/**", authenticated)
                authorize(HttpMethod.DELETE, "/books/**", authenticated)
...
이제 `POST /books`를 호출하려면 `admin / admin-secret` 으로 Basic 인증해야 합니다.
...
# 인증하여 등록 (성공)
curl -u admin:admin-secret -X POST http://localhost:8080/books \
